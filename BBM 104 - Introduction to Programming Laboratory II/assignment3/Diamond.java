...
public class Diamond extends Jewel                                      // subclass of Jewel
{
    public Diamond(String symbol, int point)
    {
        super(symbol, point);
    }
    ...
    public int[][] checkMatch(ArrayList<ArrayList<String>> grid, int x, int y)                  // override the method
    {
        if(x >= 2 && y >= 2)
        {
            if (grid.get(x - 1).get(y - 1).equals(symbol) && grid.get(x - 2).get(y - 2).equals(symbol))
            {
